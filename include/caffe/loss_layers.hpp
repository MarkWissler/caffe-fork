...
/**
 * @brief Computes the quadratic weighted kappa loss @f$
 *          TODO: verify more about the actual equation.
 *          \kappa = 1 - \frac{1-\sum_(i=1)^k\sum_(j=1)^kw_(ij)x_ij}{1-\sum_(i=1)^k\sum_(j=1)^kw_(ij)m_ij}
 *
 * Blah blah blah confusion matrix expected outcome vs actual. Probability.
 *
 * @params go here
 */
template <typename Dtype>
class QuadraticWeightedKappaLossLayer : public LossLayer<Dtype> {
 public:
  explicit QuadraticWeightedKappaLossLayer(const LayerParameter& param)
      : LossLayer<Dtype>(param),
          sigmoid_layer_(new SigmoidLayer<Dtype>(param)),
          sigmoid_output_(new Blob<Dtype>()) {}
  virtual void LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  virtual inline const char* type() const { return "QuadraticWeightedKappaLoss"; }

 protected:
  /// @copydoc QuadraticWeightedKappaLossLayer
  virtual void Forward_cpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
  virtual void Forward_gpu(const vector<Blob<Dtype>*>& bottom,
      const vector<Blob<Dtype>*>& top);
